...
// Convert string with commas → number; fallback 0
const asNumber = v =>
  ...
  return (
    <div>
      ...
      <div style={{ marginTop: 8, textAlign: 'center' }}>
        ...
        <button disabled={page >= totalPages} onClick={() => setPage(page + 1)} style={{ marginLeft: 8 }}>
          Next
        </button>
      </div>
    </div>
    ...
    <div className="modal">
      ...
      >
        ...
        <div style={{ marginBottom: 12 }}>
          ...
          <div>
            <strong>Party:</strong> {tx.party}
          </div>
          <div>
            <strong>Amount:</strong> ₹{asNumber(tx.amount).toFixed(2)}
          </div>
          {tx.billNumber && (
            <div>
              <strong>Bill No:</strong> {tx.billNumber}
            </div>
          )}
          {tx.method && (
            <div>
          ...
          {tx.checkNumber && (
            <div>
              <strong>Check No:</strong> {tx.checkNumber}
            </div>
          )}
        </div>
          ...
          {txs.map((tx, i) => {
            ...
            return (
              <tr key={tx.id || i}>
                ...
                <td>{gst}</td>
                <td>{debit  !== null ? `₹${debit.toFixed(2)}`  : '-'}</td>
                <td>{credit !== null ? `₹${credit.toFixed(2)}` : '-'}</td>
                <td>
                  ₹
                  {runningBalances[tx.id] !== undefined
                    ? asNumber(runningBalances[tx.id]).toFixed(2)
                    : '-'}
                </td>
                <td>
                  <button onClick={() => onEdit && onEdit(tx)}>Edit</button>
                </td>
                <td>
                  {tx.comment && (
...
const HomePage = () => {
  /* ------------------------------------------------------------------------ */
  /* Your original state variables, useEffect hooks, handlers, etc. go here.  */
  /* ------------------------------------------------------------------------ */

  // Example placeholders (replace with your real logic/state)
  ...
  return (
    <div className="home-page">
      {/* Example sidebar — fill with your own buttons / navigation */}
      <aside className="sidebar">
        <button className={view === 'bank' ? 'active' : ''} onClick={() => setView('bank')}>
          Bank
        </button>
      ...
      <main className="content">
        {view === 'bank' && (
          <div className="form-container">
            ...
            />
            <button onClick={handleDeposit} className="addPurchase-button">
              Deposit
            </button>
        ...
        {/* Render other views (dashboard, parties, etc.) here */}
      </main>
    </div>
  );
};
